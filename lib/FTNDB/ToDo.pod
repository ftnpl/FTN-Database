...
Once the non database table related code in FTN::Database::Nodelist, etc. is
properly separated out, make ftn-database and ftndb into being separate
distributions, creating a new repository for just FTN::Database.
...
As part of the transistion to using App::Cmd; move the functionality for the
I<create> command from bin/ftndb-admin and lib/FTNDB/Admin.pm to the new module
...
As part of the transistion to using App::Cmd; move the functionality for the
I<drop> command from bin/ftndb-admin and lib/FTNDB/Admin.pm to the new module
for the drop command.


=head2 FTNDB::Admin

Rename and move FTN::Database to FTNDB::Admin, retaining the FTN::Database
namespace for things like defining the FTN database tables?

The create_ftn_database function as currently coded really only works for
sqlite databases, because it uses an already open db handle and to get that
in for instance postgresql you need to define a database for the login.

Change the create_nodelist_table function to use the generic create_ftn_table
subroutine?
...
Add a function to enable getting the nodelist header information; it will return 
a hash, with at least the ftnyear and yearday information.  (Should really be a
function of or an extension to FTN::Nodelist.)
...
Change load command to only load new nodelist information
...
been removed and/or replaced by ftndbadm.
...
=head1 SEE ALSO
 ...
 L<ftndbadm>, L<ftndb-admim>, L<ftndb-nodelist>, L<FTNDB>, L<FTNDB::Command::create>,
  L<FTNDB::Command::drop>, L<FTN::Database>, L<FTNDB::Nodelist>, L<FTN::Database::Nodelist>
